...
func TestStatsRepositoryHostAuditsAggregatesHosts(t *testing.T) {
	...
	if !strings.Contains(body, "event_type = 'process_exec'") || !strings.Contains(body, "if(node_name != '', node_name, host_name) AS audit_host") || !strings.Contains(body, "positionCaseInsensitive(audit_host, 'node')") {
