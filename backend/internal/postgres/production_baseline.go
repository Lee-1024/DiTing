...
VALUES (
    ...
    '{
      ...
      "rules": [
        ...
        {"id":"pre-root-login-process-exit-low-risk","name":"预发忽略 root 登录进程退出","enabled":true,"conditions":[{"field":"event_type","op":"eq","value":"process_exit"},{"field":"login_username","op":"eq","value":"root"},{"field":"severity","op":"in","values":["info","low","medium"]}]},
        ...
        {"id":"pre-monitoring-agent-noise","name":"预发忽略监控探针噪声","enabled":true,"conditions":[{"field":"process_name","op":"in","values":["kube-probe","node_exporter","prometheus","telegraf","grafana-agent","zabbix_agentd","zabbix_agent2"]},{"field":"severity","op":"in","values":["info","low","medium"]}]},
        {"id":"pre-monitoring-user-noise","name":"预发忽略监控用户噪声","enabled":true,"conditions":[{"field":"username","op":"in","values":["zabbix","prometheus"]},{"field":"severity","op":"in","values":["info","low","medium"]}]},
        {"id":"pre-diting-self-vite-noise","name":"预发忽略 DiTing 自身 Vite 服务噪声","enabled":true,"conditions":[{"field":"cmdline","op":"contains","value":"/data/DiTing/"},{"field":"cmdline","op":"regex","value":"(?i)(node_modules/.bin/vite|\\bnode\\b.*\\bvite\\b)"},{"field":"severity","op":"in","values":["info","low","medium"]}]}
      ]
    }'::jsonb,
