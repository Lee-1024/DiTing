...
|---|---|---|
| Tetragon bpffs mount | `mount | grep /sys/fs/bpf` | bpffs is mounted and `/sys/fs/bpf/tetragon` can be created by Tetragon |
...
If sudo ancestry does not match, verify the sudo binary path (`/usr/bin/sudo` or `/bin/sudo`) and parent tracking support before changing policy semantics.
