...
- Sudo-aware root exclusion uses Linux audit loginuid through `current_task` data matching. DiTing should not require `--parents-map-enabled=true` by default because that flag only serves `matchParentBinaries` and can fail on some kernels when the loaded BPF object lacks `PARENTS_MAP_ENABLED`.
